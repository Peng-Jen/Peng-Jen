...
def render_board_md(board, board_for_hash, repo_url):
    ...
    for i in range(SIZE):
        ...
        for j in range(SIZE):
            ...
            if board[i][j] == HINT:
                coord = coord_str(i, j)
                url = f"{repo_url}/issues/new?title=%5BCommunity%5D+move+{coord}+{board_hash}&body=Just+click+'Create'.+You+don't+need+to+do+anything+else."
...
def write_readme(board, board_for_hash, current_player, repo_url, history=None):
    ...
    with open("README.md", "w") as f:
        ...
        if is_game_over(board_for_hash):
            ...
            f.write(f"Click [here]({REPO_URL}/issues/new?title=%5BCommunity%5D+new_game) to start a new game\n")
        else:
