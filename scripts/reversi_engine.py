...
def count_score(board):
    solid = sum(list(row).count(SOLID) for row in board)
    hollow = sum(list(row).count(HOLLOW) for row in board)
    return solid, hollow

def encode_board(board):
    import numpy as np
    board = np.array(board)
    one_hot = np.zeros((8, 8, 3), dtype=np.float32)
    one_hot[:, :, 0] = (board == 0)
    one_hot[:, :, 1] = (board == 1)
    one_hot[:, :, 2] = (board == 2)
    return one_hot

def flatten_board_indices(indices):
    return [SIZE * r + c for r, c in indices]
